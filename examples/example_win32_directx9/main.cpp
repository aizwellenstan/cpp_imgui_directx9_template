...
bool copyFile(std::string sourcePath) {
...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"COPY", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc.hInstance, NULL);
    ...
    while (!done)
    {
        ...
        if (clicked & 1)
        {
            ...
            ImGui::Text(buf1);
            copyFile(buf1);
            MessageBox(nullptr, TEXT("COPY FINISHED"), TEXT("Message"), MB_OK);
            clicked = 0;
        }
